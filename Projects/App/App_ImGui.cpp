...
namespace App_ImGui
{
	...
	void Render(gsl::not_null<ID3D12GraphicsCommandList*> cmdList)
	{
		...
		ImGui::Text("Draw   : %.2fms %.2fms", DataAverage::Get(L"•`‰æˆ—"), DataAverage::Get(L"•`‰æ‘‚«ž‚Ý") + DataAverage::Get(L"‰æ–Ê•\Ž¦") + DataAverage::Get(L"GPU‘Ò‹@"));
		ImGui::Text("  Render  : %.2fms", DataAverage::Get(L"•`‰æ‘‚«ž‚Ý"));
		ImGui::Text("  Present : %.2fms", DataAverage::Get(L"‰æ–Ê•\Ž¦"));
		ImGui::Text("  GPUwait : %.2fms", DataAverage::Get(L"GPU‘Ò‹@"));
		ImGui::Text("    GPUwait(free) : %.2fms", DataAverage::Get(L"GPU‘Ò‹@"));
		ImGui::Text("    GPUwait(busy) : %.2fms", DataAverage::Get(L"GPU‘Ò‹@"));

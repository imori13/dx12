...
void RenderObject::Create(const ModelMesh& mesh, const ModelMaterial& material, const Texture& texture, int32_t objectCount)
{
	// ヒープ生成
	{
		...
		const uint32_t heapCount = cameraCount + materialCount + lightCount + textureCount + colliderCount + objectCount;
	...
	// CameraBuffer生成
	{
		...
		m_CameraData = reinterpret_cast<CameraData*>(m_CameraBuffer.Map());
	...
	// LightBuffer生成
	{
		...
		m_LightData = reinterpret_cast<LightData*>(m_LightBuffer.Map());
	...
	// Collider
	{
		...
		DirectX::XMFLOAT4* collider = reinterpret_cast<DirectX::XMFLOAT4*>(m_ColliderBuffer.Map());
	...
	m_ColliderPipeline = ResourceManager::GetPipeline(L"Collider");
	{
		...
		m_ColliderBandle->SetGraphicsRootConstantBufferView(2, m_ColliderBuffer.GetConstantLocation());
...
void RenderObject::Initialize(const Matrix4x4& view, const Matrix4x4& projection) noexcept
{
	...
	m_LightData->LightPosition = { 0.0f,50.0f,100.0f };
	m_LightData->LightColor = { 1.0f,1.0f,1.0f };
	m_LightData->CameraPosition = { 0.0f,1.0f,2.0f };
	m_LightData->CameraDirection = { 1.0f,0.0f,0.0f };
}
